...
class ConfigurationProxy: PlatformItemSource {
    ...
    func save(to manager: inout NETunnelProviderManager) throws {
        ...
        if let data = self.data() {
            ...
            #if os(iOS)
            providerProtocol.providerBundleIdentifier = "eu.neilalexander.CupLink.extension"
            #elseif os(OSX)
            providerProtocol.providerBundleIdentifier = "eu.neilalexander.CupLinkmac.extension"
            #endif
            providerProtocol.providerConfiguration = [ "json": data ]
            providerProtocol.serverAddress = "CupLink"
            ...
            manager.saveToPreferences(completionHandler: { (error:Error?) in
                if let error = error {
                    ...
                    NotificationCenter.default.post(name: NSNotification.Name.CupLinkSettingsUpdated, object: self)
                }
            })
        }
    }
    ...
    func activityViewController(_ activityViewController: UIActivityViewController, subjectForActivityType activityType: UIActivity.ActivityType?) -> String {
        if let pubkey = self.get("PublicKey") as? String {
            return "CupLink-\(pubkey).conf.json"
        }
        return "CupLink.conf.json"
    }
    #endif
}
